 ...
 * OutputFormat format.
 *
 * {@link OutputFormatSinkFunction} can not be replaced
...
public class DtOutputFormatSinkFunction<IN> extends OutputFormatSinkFunction<IN> implements CheckpointedFunction, CheckpointListener, InputTypeConfigurable {
    ...
    protected transient ListState<FormatStateHelper> unionOffsetStates;

    protected Map<Integer, FormatStateHelper> formatStateMap;
    ...
    public void snapshotState(FunctionSnapshotContext context) throws Exception {
        FormatStateHelper formatState = ((BaseRichOutputFormat) format).getFormatState();
        if (formatState != null) {
    ...
    public void initializeState(FunctionInitializationContext context) throws Exception {
        ...
        unionOffsetStates = stateStore.getUnionListState(new ListStateDescriptor<>(
                LOCATION_STATE_NAME,
                TypeInformation.of(new TypeHint<FormatStateHelper>() {
                })));

        log.info("Is restored:{}", context.isRestored());
        if (context.isRestored()) {
            formatStateMap = new HashMap<>(16);
            for (FormatStateHelper formatState : unionOffsetStates.get()) {
