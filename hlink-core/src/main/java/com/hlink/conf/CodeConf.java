...
public class CodeConf implements Serializable {
     ...
     */
    public static CodeConf parseJob(String jobJson) {
        CodeConf config = GsonUtil.GSON.fromJson(jobJson, CodeConf.class);
        checkJob(config);
        return config;
    }
     ...
     * 校验Job配置
     *
     * @param config FlinkJobConfig
     */
    private static void checkJob(CodeConf config) {
