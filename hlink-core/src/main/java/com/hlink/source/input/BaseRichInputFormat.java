...
import com.hlink.source.ByteRateLimiter;
import com.hlink.utils.DataCodeFactoryUtil;
...
public abstract class BaseRichInputFormat extends RichInputFormat<RowData, InputSplit> {
    ...
    public void openInputFormat() throws IOException {
        ...
        if (useCustomReporter()) {
            customReporter = DataCodeFactoryUtil.discoverMetric(config, getRuntimeContext(), makeTaskFailedWhenReportFailed());
            customReporter.open();
        }
    ...
    private void initRestoreInfo(){
        if(formatState == null){
            formatState = new FormatStateHelper(indexOfSubTask, null);
        } else {
     ...
     * @return
     */
    public FormatStateHelper getFormatState() {
